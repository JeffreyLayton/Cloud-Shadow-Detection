...
// Quick and dirty fix (TODO: Make path not traverse parent directory)
...
void Window::keyMetaCallback(GLFWwindow *window, int key, int scancode, int action, int mods) {
    CallbackInterface *callbacks
        = static_cast<CallbackInterface *>(glfwGetWindowUserPointer(window));
...
Window::Window(
    ...
    if (callbacks != nullptr) { connectCallbacks(); }
